...
int main(void)
{
     ...
     * ------------------------------------------------------------------ */
    while (1) {
        /* 4 — Read B1 and control LD2.
         *
         * Steps:
         *   a) Read gpio_read(B1_PORT, B1_PIN).
         *   b) Write the result with gpio_write(LD2_PORT, LD2_PIN, ...).
         */
        int button_state = gpio_read(B1_PORT, B1_PIN);
        gpio_write(LD2_PORT, LD2_PIN, button_state);
